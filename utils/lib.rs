...
lazy_static! {
    ...
    pub static ref RANDOM_100_000_FEW: Vec<usize> = {
        let mut rng: StdRng = StdRng::from_seed(RNG_SEED);
        (0..100_000).map(|_| {
            rng.gen_range(0, SMALL_KEYS)
        }).collect()
    };
    pub static ref RANDOM_10_000_FEW: Vec<usize> = {
        let mut rng: StdRng = StdRng::from_seed(RNG_SEED);
        (0..10_000).map(|_| {
            rng.gen_range(0, SMALL_KEYS)
        }).collect()
    };
    pub static ref RANDOM_1000_FEW: Vec<usize> = {
        let mut rng: StdRng = StdRng::from_seed(RNG_SEED);
        (0..1000).map(|_| {
            rng.gen_range(0, SMALL_KEYS)
        }).collect()
    };
    pub static ref RANDOM_100_FEW: Vec<usize> = {
        let mut rng: StdRng = StdRng::from_seed(RNG_SEED);
        (0..100).map(|_| {
            rng.gen_range(0, SMALL_KEYS)
        }).collect()
    };
    pub static ref RANDOM_10_FEW: Vec<usize> = {
        let mut rng: StdRng = StdRng::from_seed(RNG_SEED);
        (0..10).map(|_| {
            rng.gen_range(0, SMALL_KEYS)
        }).collect()
...
lazy_static! {
    ...
    pub static ref SORTED_100_000_FEW: Vec<usize> = {
        (0..50).flat_map(|v| iter::repeat(v).take(2000)).collect()
    };
    pub static ref SORTED_10_000_FEW: Vec<usize> = {
        (0..50).flat_map(|v| iter::repeat(v).take(200)).collect()
    };
    pub static ref SORTED_1000_FEW: Vec<usize> = {
        (0..50).flat_map(|v| iter::repeat(v).take(20)).collect()
    };
    pub static ref SORTED_100_FEW: Vec<usize> = {
...
lazy_static! {
    ...
    pub static ref REVERSE_100_000_FEW: Vec<usize> = {
        (0..50).rev().flat_map(|v| iter::repeat(v).take(2000)).collect()
    };
    pub static ref REVERSE_10_000_FEW: Vec<usize> = {
        (0..50).rev().flat_map(|v| iter::repeat(v).take(200)).collect()
    };
    pub static ref REVERSE_1000_FEW: Vec<usize> = {
        (0..50).rev().flat_map(|v| iter::repeat(v).take(20)).collect()
    };
    pub static ref REVERSE_100_FEW: Vec<usize> = {
